...
Below is a **frequentist, R-centric framework** for **simulation-based design of clinical trials with a binary primary outcome**. The structure parallels the continuous and ordinal cases, but focuses on the **distinct design sensitivities of Bernoulli data**, including nonlinearity, rare events, and estimand choice.
...
* **Marginal treatment effect** (from GLMs)
* **Number needed to treat (NNT)** (derived)

### **Simulation Role**

* Generate outcomes under a known data-generating model
* Analyze using alternative links (logit, log, identity)
* Evaluate:
  * Bias
  * Power
  * Confidence interval coverage
  * Interpretability under realistic event rates

### **R Tools**

* stats::glm(family = binomial)
* sandwich**, **emmeans
* Custom Monte Carlo loops

---

## **2. Baseline Event Rate Sensitivity**

### **Why It Matters**

Binary trial power is **extremely sensitive** to control-arm event rates.

### **Simulation Scenarios**

* Overestimated vs underestimated control risk
* Event rates varying by site or subgroup
* Time-varying baseline risk (for long trials)

### **Outputs**

* Power curves vs assumed baseline risk
* Bias when baseline risk is misspecified
* Robustness of conclusions across plausible ranges

---

## **3. Rare Event and Low-Count Scenarios**
...
## **4. Covariate Adjustment and Efficiency Gains**

### **Why Simulate**

Covariate adjustment can yield meaningful power gains—but only if done correctly.

### **Scenarios**

* Strong vs weak prognostic covariates
* Nonlinear covariate effects
* Imbalance despite randomization

### **Outputs**

* Relative efficiency vs unadjusted analysis
* Bias under misspecification
* Type I error preservation

### **R Tools**

* glm(outcome ~ treatment + covariates)
* rms::lrm

---

## **5. Model Misspecification and Link Function Robustness**

### **Design Concern**

The logistic link is not always the most interpretable or stable.

### **Simulations Compare**

* Logistic (OR)
* Log-binomial (RR)
* Modified Poisson with robust SEs
* Linear probability model (RD)

### **Outputs**

* Convergence failure rates
* Bias and CI coverage
* Practical tradeoffs for reporting

### **R Tools**

* glm(family = binomial(link="log"))
* glm(..., family = poisson)** + **sandwich

---

## **6. Missing Data and Intercurrent Events**

### **Binary-Specific Issues**

* Missingness often outcome-dependent
* Composite endpoints with competing events
* Treatment discontinuation affecting outcome ascertainment
...
* Complete case
* Multiple imputation (binary)
* Conservative imputation (worst-case)
* Tipping-point analysis

### **R Tools**

* mice
* Custom pattern-mixture simulations

---

## **7. Composite and Co-Primary Binary Endpoints**

### **Why This Is Tricky**

* Event components differ in frequency and importance
* Treatment effects may be heterogeneous

### **Simulation Goals**

* Evaluate power loss/gain from composite construction
* Quantify dominance of one component
* Assess interpretability risk

### **Outputs**

* Component-specific event rates
* Marginal vs joint power
* Sensitivity analyses

---

## **8. Subgroup and Effect Heterogeneity**

### **Simulation Uses**

* Explore qualitative vs quantitative interactions
* Evaluate false positive subgroup findings
* Design multiplicity control strategies

### **Outputs**

* Interaction power
* Inflation of Type I error
* Stability of subgroup estimates

---

## **9. Interim Analyses and Group Sequential Designs**

### **Binary-Specific Issues**

* Non-normal test statistics
* Information fraction depends on accrued events

### **Simulations Evaluate**

* Alpha spending under binomial data
...
* gsDesign
* Custom simulation wrappers around interim GLMs

---

## **10. Non-Inferiority and Equivalence Designs**

### **Key Challenges**

* Choosing margins on RD vs RR vs OR scale
* Event-rate dependence of margins

### **Simulation Role**

* Validate Type I error near the margin
* Compare margin scales
* Explore sensitivity to control risk drift

---

## **11. Clustered and Multicenter Designs**
...
* Power loss due to clustering
* Robustness of mixed vs marginal models
...
## **12. Estimator Stability and Operational Risk**
...
* Bias conditional on convergence
* Practical design recommendations

---

## **Recommended Simulation Outputs (Binary Trials)**

Across all scenarios, report:
...
* NNT distribution (where appropriate)
...
| **Issue**     | **Why Simulate**            |
| ------------------- | --------------------------------- |
| Baseline risk       | Dominant power driver             |
| Rare events         | Separation and bias               |
| Estimand choice     | Interpretation & stability        |
| Missingness         | Often outcome-dependent           |
| Composite endpoints | Hidden power/interpretation traps |
| Interim looks       | Non-Gaussian behavior             |

---

## **Bottom Line**

For binary outcomes, **simulation is essential** to:

* Validate assumptions about event rates
* Choose an estimand that is both interpretable and statistically stable
* Anticipate rare-event and convergence failures
* Defend design decisions to regulators and DSMBs

This is a robust foundation. To make this framework truly comprehensive for modern clinical development, we need to extend it into  **adaptive designs** ,  **causal estimands (ICH E9 R1)** , and  **longitudinal binary structures** .

The unique challenge with binary data is that the **variance is tied to the mean** (**$Var = p(1-p)$**). This creates a feedback loop where misspecifying the event rate mis-sizes the variance, and thus the sample size.

Here are **5 Advanced Extensions** to your framework, followed by a **Concrete R Simulation Template** to implement the core logic.

---

### 13. Adaptive Sample Size Re-estimation (SSR)

Why It Matters

In binary trials, if your estimated control rate ($p_c$) is off, your power calculation is wrong because the variance is wrong. (Unlike normal data where variance $\sigma^2$ can be independent of the mean $\mu$).

**Simulation Scenarios**

* **Blinded SSR:** Re-estimating the nuisance parameter (overall event rate) at an interim to adjust **$N$** without unblinding.
* **Unblinded SSR:** Promising Zone designs (increasing **$N$** if conditional power is moderate).

**Outputs**

* Distribution of final sample sizes (**$N_{final}$**).
* Type I error inflation check (crucial for unblinded adaptations).
* Probability of "futility" stops vs. "success" expansions.

**R Tools**

* `rpact` (best-in-class for regulatory adaptive designs)
* `gsDesign`

### 14. Causal Estimands and Intercurrent Events (ICH E9 R1)

Design Question

How do we handle binary outcomes when patients die, switch treatments, or drop out before the assessment?

**Simulation Scenarios**

* **Treatment Policy Strategy (ITT):** Simulate post-discontinuation data (e.g., return to baseline risk).
* **Hypothetical Strategy:** Impute outcomes "as if" the patient continued treatment.
* **Composite Strategy:** Incorporating death/discontinuation into the binary failure definition.

**Outputs**

* Sensitivity of the Treatment Effect to the imputation method (e.g., Does "Worst Case Imputation" kill the power?).
* Bias comparisons between "tipping point" analyses.

### 15. Longitudinal Binary Outcomes (Repeated Measures)

Why It Matters

Many binary trials measure "Success" at Month 3, 6, and 12. Using only Month 12 discards data.

**Methods Compared**

* **GEE (Generalized Estimating Equations):** For population-averaged effects (marginal).
* **GLMM (Generalized Linear Mixed Models):** For subject-specific effects (conditional).

**Simulation Goals**

* Compare power of a single time-point analysis vs. longitudinal analysis (using `glmer` or `geepack`).
* Assess impact of correlation structures (Exchangeable vs. AR1) on standard errors.
...
### 16. Bayesian Assurance (Probability of Success)

Why It Matters

Frequentist power assumes the effect size is exactly X. Bayesian Assurance simulates power over a distribution of likely effect sizes and control rates (priors).

**Simulation Scenarios**

* Prior on **$p_{control}$**: Beta distribution based on historical data.
* Prior on Treatment Effect: Scepical vs. Enthusiastic priors.

**Outputs**

* **Unconditional Probability of Success:** A single metric weighing power by the likelihood of the assumptions.

### 17. Visualization of Binary Design Space

Why It Matters

Binary power is non-linear. A table isn't enough to see the "cliff" where power drops off.

**Visual Outputs**

* **Heatmaps:** X-axis = Control Rate, Y-axis = Odds Ratio, Color = Power.
* **Volcano Plots:** Visualizing Type I error vs Power across thousands of simulation iterations.

---

### Practical R Simulation Template for Binary Outcomes
